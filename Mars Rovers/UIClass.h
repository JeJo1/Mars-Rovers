...
public:
	...
	void Output()
	{
		...
		int totalWaiting = Mnum + Enum + Pnum;
		 ...
		 {
			 cout << MMWL.getItemAt(i)<<",";
		 }
		 cout << MMWL.getItemAt(Mnum-1) << "}"<<endl;
		 cout << "-----------------------------------------------------------------------------------" << endl;
		 int countMPL= MPL.getLength();
		 cout << countMPL<< " In-Excution Mission/Rover: [" << endl;
		 countMPL--;
		 ...
		 if (dynamic_cast<EmergencyMission*>(MPL.getItemAt(countMPL)))
			 ...
			 cout << MPL.getItemAt(countMPL)->getID() << "/" << RPL.getItemAt(countMPL)->getN() << "}" << endl;
			 cout << "-----------------------------------------------------------------------------------" << endl;
